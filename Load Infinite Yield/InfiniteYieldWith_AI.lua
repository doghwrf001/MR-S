--!nonstrict
--[[ 
    SMART INFINITE YIELD - V1.3.2 STABLE
    - Architecture: Context Awareness + Event-Based Bridge
    - Logic: VolQ5 & Pollinations.AI (LLM Model)
    - Status: Stable 
    - v1.3.0: Added sound feedback config, tour config, plugin bridge ready
]]
...
local function waitForBridge(timeout, retryCount)
...
-- Get bridge connection status for debugging
...
--- @return TextButton
local function createButton(text, props)
    local defaults = {
    ...
    local btn = createInstance("TextButton", defaults)
    addCorner(btn, 4)
    return btn
end
...
local function createLabel(text, props)
    ...
    return createInstance("TextLabel", defaults)
end
...
--- @return Frame
local function createFrame(props, cornerRadius, strokeColor)
...
local FastMap = {
    ...
    ["^touchinterests$"] = "touchinterests", ["^touchinterest$"] = "touchinterests",
    ["^firetouchinterests$"] = "touchinterests", ["^firetouchinterest$"] = "touchinterests",
...
do
    for pattern, command in pairs(FastMap) do
        table.insert(OrderedFastMap, { Pattern = pattern, Command = command })
    end
    -- Sort by pattern length descending (longer = more specific = higher priority)
    table.sort(OrderedFastMap, function(a, b)
...
local function queryAI(promptText, retryContext, attemptCount)
    ...
    if not IS_CHAT_MODE and not retryContext then
        ...
        if not isChain then
            ...
            -- Second try: OrderedFastMap lookup (pattern-based fallback)
...
setNamespaceValue("Version", "1.3.1")
...
local PluginBridge = {
    ...
    executeCommand = function(command)
        if IY_Interface and IY_Interface.execCmd then
            IY_Interface.execCmd(command)
